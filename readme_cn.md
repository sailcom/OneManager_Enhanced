...
2. 新增 `GuestUpload` 与 `GuestView` 配置项：
    ...
    - `GuestView`：控制是否允许游客浏览目录。
3. 引入统一的文件重命名机制，所有上传方式的图片均会自动重命名。
4. 权限分级控制：
    - 游客仅允许上传图片类型文件，以降低安全风险；
    - 管理员上传文件类型不受限制。
5. 由于原项目主题风格较多，无法逐一适配，仅对本人使用的 `classic`、`new_onedrive`、`renexmoe` 三种风格进行了调整，其他风格未作修改。
